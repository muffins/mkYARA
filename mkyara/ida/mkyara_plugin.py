...
def get_arch_info():
    ...
    if proc == "metapc":
        return CS_ARCH_X86, instruction_mode
...
class YaraRuleDialog(QtWidgets.QDialog):
    ...
    def populate_form(self):
        ...
        self.layout.addWidget(
            QtWidgets.QLabel(
                "Generated Yara rule from 0x{:x} to 0x{:x}".format(
...
class mkYARAPlugin(idaapi.plugin_t):
    ...
    def init(self):
        loose_yara_action = idaapi.action_desc_t(
            ...
            generic_handler(
                lambda: self.generate_yara_rule("loose")
            ),  # The action handler.
        ...
        normal_yara_action = idaapi.action_desc_t(
            ...
            generic_handler(
                lambda: self.generate_yara_rule("normal")
            ),  # The action handler.
        ...
        strict_yara_action = idaapi.action_desc_t(
            ...
            generic_handler(
                lambda: self.generate_yara_rule("strict")
            ),  # The action handler.
            None,  # Optional: the action shortcut
        ...
        data_yara_action = idaapi.action_desc_t(
            ...
            generic_handler(
                lambda: self.generate_yara_rule("normal", is_data=True)
            ),  # The action handler.
            None,  # Optional: the action shortcut
